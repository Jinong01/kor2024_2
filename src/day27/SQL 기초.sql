...
# [5] db server에는 여러개 데이터베이스가 존재하므로 사용/활성화 할 데이터베이스 선택
use myDBtest1;

# [실습1] - 아래와 같이 순서대로 SQL문을 작성하여 실행하시오.
drop database if exists test1;	-- 1. 'test1' 이름의 데이터베이스가 존재하면 삭제하기
...
/*
    ...
    [ 테이블 생성 시 필드(데이터) 타입 = JAVA의 데이터타입 역할이 같다. oracle vs mysql ]
		[ 정수 ]
		...
		[ 실수 = 부동소수점, 오차 존재 ]
			FLOAT	4바이트
            DOUBLE	8바이트
        [ 문자타입으로 저장되는 숫자, 오차 없는 실수 표현 ]
			DECIMAL
		[ 날짜 ]
			DATE		날짜 표현, 1000년 ~ 9999년, ex)0000-00-00
            DATETIME	날짜와 시간 표현 ex)0000-00-00 00:00:00
            TIME		시간 표현, 00:00:00
		[ 문자열 ]
			1. CHAR(문자길이)		: 고정길이 , 최대 1~255글자 , ex)char(5) --> "유재석" --> [유][재][석][][]
            2. VARCHAR(문자길이)	: 가변길이 , 최대 1~255글자 , ex)varchar(5) --> "유재석" --> [유][재][석]
            3. TEXT				: 최대길이 6만개 글자 표현
            4. LONGTEXT			: 최대길이 42억개 글자 표현(약4GB)
		[ 논리 ]
...
create table member(
    ...
    논리필드 bool #마지막에는 , 넣지 않는다.
    );									# 6. 테이블 생성 : 테이블명 (속성명 속성타입, 속성명 속성타입,...); 
show tables; # 7. 테이블 목록 보기
select * from member; # 8. 지정한 테이블의 레코드 보기

/*
	[SQL과제] 키오스크 개발 - 필요한 데이터베이스 구축 하기
		- 참고영상 : https://www.youtube.com/watch?v=ksuYwD1oC3A
        - 요구 사항
			1. 여러개 카테고리 중에 하나의 카테고리를 선택하여 카테고리별 제품들을 출력
            2. 해당 제품을 선택하여 수량 입력받아 주문 처리 - 단) 한번 주문에 여러개 제품을 주문할 수 있다.
            3. 주문번호를 받아서 대기한다.
		- 메모리 설계 필수 필드 속성
			1.카테고리명 2.제품명 3.가격 4.주문수량 5.주문날짜
		- 핵심 : 여러 테이블 구성할때는 테이블간의 데이터 중복을 최소화 한다.
